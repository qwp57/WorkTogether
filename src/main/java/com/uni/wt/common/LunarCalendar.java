...
public class LunarCalendar {
    ...
    public ArrayList<String> WeekHoliday(String yyyymmdd) {
		ArrayList<String> weekHoliday = new ArrayList<String>();
		
		
		log.info("올해 연도 {}", yyyymmdd.substring(0, 4));
		Set<String> yearholiday = holidayArray(yyyymmdd.substring(0, 4));//올해 휴일 
		
		LocalDate weekstart= LocalDate.parse(yyyymmdd, DateTimeFormatter.BASIC_ISO_DATE);
		LocalDate weekend = weekstart.plusDays(6);
		log.info("[weekstart 일요일] : {}",weekstart.toString());
		log.info("[weekend 토요일] : {}",weekend.toString());
	
		String weekendstr = weekend.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
		yearholiday.add(yyyymmdd);// 시작날짜 == 일요일
		yearholiday.add(weekendstr);
		

		//[20220508, 20220606, 20220815, 20220505, 20220912, 20220911, 20221009, 20220910, 20220301, 20221225, 20220202, 20220201, 20220101, 20221003, 20220131, 20220909]
		for(String y : yearholiday) {
			
			
			if( Integer.parseInt(y) >= Integer.parseInt(yyyymmdd)&& Integer.parseInt(y) <= Integer.parseInt(weekendstr)) {
